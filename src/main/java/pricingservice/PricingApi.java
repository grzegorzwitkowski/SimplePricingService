...
public class PricingApi {
    ...
    public PricingApi(FeeCalculator feeCalculator, PriceListRepository priceListRepository, PriceCalculationRepository priceCalculationRepository) {
    ...
    public PriceCalculation calculatePrice(Set<PromoOption> selectedPromoOptions, Set<Integer> categoryPath) {
        BigDecimal totalPrice = feeCalculator.calculateTotalPrice(selectedPromoOptions, categoryPath);
        PriceCalculation priceCalculation = new PriceCalculation(UUID.randomUUID(), totalPrice);
        priceCalculationRepository.add(priceCalculation, priceCalculation.getCalculationId());
